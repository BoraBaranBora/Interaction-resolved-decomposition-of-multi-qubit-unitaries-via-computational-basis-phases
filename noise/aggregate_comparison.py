"""Aggregate reference/robust OU runs using the original single-axis plot style."""
...
def plot_gate(
    ...
    *,
    ...
    grouped = grouped_curves(robust_rows, gate)
            ...
            rows.append(
                {
                    ...
                    "reference_noise_loss": r0[
                        "dephasing_induced_average_fidelity_loss"
                    ],
                    "robust_noise_loss": r1[
                        "dephasing_induced_average_fidelity_loss"
                    ],
                }
            )
...
def write_latex_table(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    lines = [
        r"\begin{table*}[t]",
        r"\centering",
        r"\caption{Average gate fidelities in the three experimentally anchored material settings. Reference and exposure-penalized pulses use identical noise seeds; uncertainties are one standard error.}",
    ...
    for scenario_id in SCENARIO_ORDER:
        ...
        lines.append(
            ...
            f"{cell(zzz, 'reference')} & {cell(zzz, 'robust')} & "
            f"{cell(xzz, 'reference')} & {cell(xzz, 'robust')} \\\\"
        )
...
def main() -> None:
    ...
    for gate in GATES:
        ...
        # Keep the old manuscript filename as a byte-identical compatibility copy.
        shutil.copy2(
            canonical,
            figure_dir / f"ou_fidelity_sweep_{gate}_reference_vs_robust.png",
        )
    ...
    (table_dir / "comparison_metadata.json").write_text(
        json.dumps(
            {
                "plotted_variant": "electron-exposure-penalized",
                "reference_variant_usage": "experimental-scenario comparison table",
                "cartesian_grid": {
