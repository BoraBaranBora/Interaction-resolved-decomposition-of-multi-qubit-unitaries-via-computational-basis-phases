...
The stochastic term is inserted inside every short-time propagator. The OU
width is calibrated exclusively to an electron Ramsey T2*. No Hahn-echo,
dynamical-decoupling, or nuclear-noise channel is introduced.

Example
-------
python -m noise.run \
    --gate diagonal \
    --pulse-dir results/control_zzz_direct \
    --t2-star-us 5 \
    --tau-c-us 15 \
    --n-realizations 512 \
    --output-dir results_ou/example
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--t2-star-us", type=float, required=True)
    parser.add_argument("--tau-c-us", type=float, required=True)
    ...
    parser.add_argument(
        ...
        type=str,
        default="{}",
        help="JSON object copied into the output summary by the sweep runner.",
    )
...
def main() -> None:
    args = parse_args()
    if args.t2_star_us <= 0.0:
        raise ValueError("--t2-star-us must be positive")
    if args.tau_c_us <= 0.0:
        raise ValueError("--tau-c-us must be positive")
    ...
    params = OUParameters.from_t2_star(
        args.t2_star_us * 1e-6,
        args.tau_c_us * 1e-6,
    )
    ...
    print("Calibration: electron Ramsey T2* only")
    print(f"Pulse checkpoint: {checkpoint_path}")
    print(f"Gate: {args.gate}")
    print(f"T2*: {args.t2_star_us:g} us")
    print(f"tau_c: {args.tau_c_us:g} us")
    print(f"sigma: {params.sigma_rad_s:.9e} rad/s")
    ...
    regime_metadata = json.loads(args.regime_metadata_json)
    ...
    summary: dict[str, Any] = {
        ...
        "noise_model": {
            ...
            "t2_star_us": args.t2_star_us,
            "correlation_time_us": args.tau_c_us,
